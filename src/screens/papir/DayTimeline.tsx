...
const yFor = (decimalHour: number) => (decimalHour - START_HOUR) * HOUR_HEIGHT;

export type TimelineEvent = { start: number; end?: number; title: string; place?: string };

const DEMO_EVENTS: TimelineEvent[] = [
  { start: 11.0, end: 12.0, title: 'Kundemøde hos Hansen', place: 'Hovedgaden 12' },
  { start: 13.92, end: 14.25, title: 'Aflever 2 dyr til Ole', place: 'Gården' },
  { start: 16.5, end: 17.0, title: 'Opkald med revisor', place: 'Telefon' },
];

type Props = { events?: TimelineEvent[]; now?: Date };

export function DayTimeline({ events = DEMO_EVENTS, now: nowProp }: Props) {
  ...
  const nowClamped = Math.min(Math.max(nowDecimal, START_HOUR), END_HOUR);
  const nowY = yFor(nowClamped);

  const totalHeight = (END_HOUR - START_HOUR) * HOUR_HEIGHT + 40;
  const hours = useMemo(
  ...
  return (
    <View style={{ height: totalHeight, paddingTop: 8 }}>
      ...
      <View style={[styles.rail, { top: 0, height: nowY, backgroundColor: papirColor.red }]} />

      {/* Hour rows: timestamp + gridline + node */}
      {hours.map((h) => {
        const y = yFor(h);
        const past = h <= nowDecimal;
        return (
      ...
      {events.map((ev, i) => {
        ...
        const dur = (ev.end ?? ev.start + 0.75) - ev.start;
        const height = Math.max(dur * HOUR_HEIGHT - 10, 46);
        return (
          <View key={i} style={[styles.event, { top, height }]}>
            <Text style={styles.eventTitle}>{ev.title}</Text>
            {ev.place ? <Text style={styles.eventPlace}>{ev.place}</Text> : null}
          </View>
        );
      })}
