...
//
// In-app log of every mail Zolva has sent on behalf of the current user,
// across all providers (Gmail, Outlook, iCloud). Exists primarily because
// iCloud-sent mail does not land in Apple Mail's Sent folder (see
// 2026-05-06-icloud-send-mail-design.md). Provider-uniform UX so the
// user has one mental model: "what has Zolva sent for me?".
...
export function SentMailScreen({ onClose }: Props) {
  ...
  const reload = useCallback(async () => {
    ...
    const list = await listSentMails(userId);
    setRecords(list);
  }, [userId]);
  ...
  useEffect(() => {
    const unsub = subscribeSentMails((changedUserId) => {
      if (changedUserId === userId) reload();
    });
    return unsub;
  }, [userId, reload]);
  ...
  const handleClear = useCallback(() => {
    Alert.alert(
      'Ryd sendte mails?',
      'Listen i Zolva slettes — selve mailene er ikke påvirket og forbliver i din mailudbyders Sendt-mappe (på Gmail og Outlook).',
      [
    ...
    <View style={{ flex: 1, position: 'relative', backgroundColor: t.paper }}>
      ...
      >
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ ...type.eyebrow, color: t.ink3 }}>SENDTE FRA ZOLVA</Text>
          </View>
...
function SentRow({ record, expanded, onToggle, showDivider }: RowProps) {
  ...
  const providerLabel = providerDisplayName(record.provider);
  ...
  return (
    ...
    >
      ...
      <Text
        style={{ ...type.body, color: t.ink2, marginTop: 2 }}
        numberOfLines={1}
      >
        Til: {recipients}
      </Text>
...
function providerDisplayName(p: SentMailRecord['provider']): string {
  switch (p) {
...
function groupByDay(records: SentMailRecord[]): Group[] {
  ...
  // Newest day first; records inside each day already newest-first because
  // recordSentMail unshifts into the cache.
